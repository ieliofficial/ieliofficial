# Hi, I'm Eli 👋

**CTO & Founder** at [Codrlabs](https://codrlabs.com) • **Full-Stack Developer** • **Musician**

Building software that solves real problems. Leading an experimental music project. Writing about tech and creativity.

## 🚀 What I'm Working On

**[Codrlabs](https://codrlabs.com)** — Custom software development for small businesses and creators. We build web apps, mobile solutions, and digital products that help our clients grow.

**[SpeakshApp](https://codrlabs.com/speakshapp)** — Personalized micro-learning app for English learners. Build natural fluency in just 10-15 minutes daily through intelligent practice, adaptive spaced repetition, and sentence patterns. No rigid lessons—just real-world progress.

**[Maestronom](https://codrlabs.com/maestronom)** — Distraction-free practice companion for musicians. Professional metronome with exclusive "Leaps" training mode, multi-instrument tuner, chord library, and interactive music theory tools. Built by musicians, for musicians.

**[Edmyria](https://edmyria.bandcamp.com/)** — Experimental music project blending diverse genres and cultural influences. Released EPs: *Towards Infinity* and *Edmyria (Self-titled)*.

## 📝 Writing

I write about software development, problem-solving, and technology on the [Codrlabs blog](https://codrlabs.com/blog).

## 🎸 Creative Work

Music has always been a parallel passion. As founder and lead guitarist of **Edmyria**, I compose, produce, and mix experimental tracks that push genre boundaries.

- [Listen on Bandcamp](https://edmyria.bandcamp.com/)
- [Watch on YouTube](https://www.youtube.com/Edmyria)

## 🤝 Open Source & Community

Active contributor to [Nasqueron](https://devcentral.nasqueron.org/), supporting open-source projects and collaborative development.

**[Codrlabs Open](https://github.com/codrlabs)** — Open-sourcing projects, mentoring developers, and building community. Currently working on **Vizably** and other initiatives to help people grow their skills.

## 📫 Let's Connect

- **Email:** [eli@eon.observer](mailto:eli@eon.observer)
- **LinkedIn:** [linkedin.com/in/ieli](https://www.linkedin.com/in/ieli/)
- **Work Email:** [hello@codrlabs.com](mailto:hello@codrlabs.com)

---

*Based in Edmonton, Alberta 🇨🇦 • Building software and making music*
